...
class PythonDeps {
    ...
    afterArtifacts() {
        ...
        const save_stats_src = path.join(this.serverless.config.servicePath, 'save_stats.py');
        const calc_tiers_src = path.join(this.serverless.config.servicePath, 'calc_tiers.py');
        const save_stats_dist = path.join(distPath, 'save_stats.py');
        ...
        console.log(`Copying ${save_stats_src} into ${save_stats_dist}`);
        fs.copySync(save_stats_src, save_stats_dist);
